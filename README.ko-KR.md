...
0.5.1은 기존 구조화 워크플로에 한국어 트리거, 사용자 언어 추종, 한중 승인 의미 대조 테스트를 추가했습니다. 그러나 모든 모델, 플랫폼 진입점, 랜덤 시드, 예술 스타일, 실제 사용자 경로가 검증되었다는 뜻은 아닙니다. 플랫폼 기능은 바뀔 수 있으므로 모델 길이, 소재 수, 음향 또는 편집 기능에 관한 판단에는 출처, 모델, 진입점, 확인 날짜를 기록해야 합니다.
