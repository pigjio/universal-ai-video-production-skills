...
| 对白、画外声、声音桥、沉默或可见口型 | `references/dialogue-audio-and-lipsync-contract.md` | 声源/听者/原文/语言/触发点、读演估时与生成或后期分工 |
...
5. **摄影和声音**：运镜写初始机位、触发、运动路径、停稳/切点；固定镜头直接写固定。音轨仅在任务需要时写，未知生成能力留问项，不保证模型产出。普通结束状态融入镜头结果，只有真实续接或关键末帧职责才单列接口。
   - 有声音职责时按声音参考展开可复制文本，不只引用内部 cue ID。可见嘴部的后期配音不自动等于口型同步；未知能力列待核验，不擅自改成旁白。
   - 复杂接触、遮挡或密集动作需先转生产阶段的生成风险判断；改变镜数、机位或关键可见性先回分镜确认，不能用“更容易生成”暗改剧情。
...
视频提交块采用 `references/seedance-student-delivery.md` 的显式逐镜结构：代码块直接从镜头编号开始，不添加请求级公共头，逐镜保留镜头编号、镜头任务、目标时长、景别与机位、画面构图、起始状态、动作与表演、时间轴、摄影机运动、物理与空间约束、声音、结束状态、切点与衔接。不能只在代码块外给分镜表，也不能改成无标题长段落；多镜请求逐镜完整展开，不用“同上”。不适用标明原因，未知能力标待核验；韩文按同结构翻译字段标题。

固定交付末行：每个可复制视频提示词代码块最后一个非空行必须为“无背景音乐”（不加引号或句号），所有其他要求写在它之前；韩文稿也保留该中文原文。正文不要求背景配乐，但保留所需对白、动作音效和环境声。逐请求核对，不得只放在代码块外或整份文档末尾；该约束不代表已验证实际生成音轨。
...
局部修订后对照相邻请求的末态与首态。已在前镜完成的观察、接触、换手或台词，后镜应写成保持结果，不重复发起；若原下游开场动作被提前，不能仅说“其他不变”。先保留原接口，或明确提议最小下游修订并遵守保护范围。关键动作移出画框不等于取消：若它决定后镜起点，在当前独立复制块中写清同场画外发生的动作与末态，并禁止为展示该动作擅自切回；不能只在块外说明中交代。

Seedance 的时长、素材数量、输入模态和字符限制是动态能力。只采用带官方来源、核验日期、模型、产品入口和模式的当前证据；官方案例归纳、第三方经验与项目实测分别标注，均不冒充永久官方语法。

## 5. 公共头与输出模板

以下是记录模板，不把行政字段直接粘入平台提示词。`version` 是该制品修订号，技能版本为 0.7.0。引用素材的 ID 不含版本。空列表表示尚无证据，不代表已核验。
